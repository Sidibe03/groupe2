...
def recuperer():
    ...
    cursor = con.cursor()
    cursor.execute("select * from fournisseur where id='" + idchamp.get() + "'")
...
def ajouter():
    ...
    if (id == " " or nom == " " or prenom == " " or tel == " " or typro == " " or adresse == " "):
        ...
        con = mysql.connect(host="localhost", user="root", password="", database="bigstock")
        cursor = con.cursor()
        cursor.execute("insert into fournisseur values('" + id + "','" + nom + "','" + prenom + "','" + tel + "','" + typro + "','" + adresse + "') ")
        cursor.execute("commit")
...
def supprimer():
    if (idchamp.get() == " " ):
        ...
        con = mysql.connect(host="localhost", user="root", password="", database="bigstock")
        cursor = con.cursor()
        cursor.execute("delete from fournisseur where id='" + idchamp.get() + "' ")
        cursor.execute("commit")
        afficher()
        con.close()
...
def modifier():
    ...
    con = mysql.connect(host="localhost", user="root", password="", database="bigstock")
    cursor = con.cursor()
    cursor.execute("update fournisseur set id='" + id + "',nom='" + nom + "',prenom='" + prenom + "',tel='" + tel + "',typro='" + typro + "',adresse='" + adresse + "',where id='" + id + " '")
    cursor.execute("commit")
...
texte=Label (fenetre5,text="Les Fournisseurs",font=('Calistoga',25),bg="white")
